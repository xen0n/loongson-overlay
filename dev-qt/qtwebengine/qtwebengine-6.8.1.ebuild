...
PATCHSET_LOONG="f85578f0a140fa8b534e7150a0ed1b9db4277ff9"
PATCHSET_LOONG_PV="6.8.1"
SRC_URI+="
	https://dev.gentoo.org/~ionen/distfiles/${PN}-6.8-patchset-7.tar.xz
	loong? (
               https://github.com/AOSC-Dev/chromium-loongarch64/archive/${PATCHSET_LOONG}.tar.gz -> chromium-loongarch64-aosc-patches-${PATCHSET_LOONG}.tar.gz
	)
"
...
src_prepare() {
	...
	if use loong ; then
		...
		local other_patches_to_apply=(
			rollup
			ninja-race-condition
		)
		for p in "${other_patches_to_apply[@]}"; do
			eapply -p2 "${WORKDIR}/chromium-loongarch64-${PATCHSET_LOONG}/qt6-webengine/qt6-${PATCHSET_LOONG_PV}"."${p}".diff
		done
                sed -i '1,12d' "${WORKDIR}/chromium-loongarch64-${PATCHSET_LOONG}/qt6-webengine/qt6-${PATCHSET_LOONG_PV}".loongarch64.diff
		for p in "${WORKDIR}/chromium-loongarch64-${PATCHSET_LOONG}/qt6-webengine/qt6-${PATCHSET_LOONG_PV}".loongarch64*; do
			eapply -p2 "${p}"
		done
		eapply "${FILESDIR}"/${PN}-6.8.1-loong.patch
